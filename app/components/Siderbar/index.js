...
export default function Sidebar({ isSidebarOpen, onClose }) {
  ...
  return (
    ...
    <motion.div
      className={`${fonts?.poppins.className} overflow-x-scroll sidebar-content fixed top-0 left-0 h-full w-64 sm:w-72 bg-gray-200 shadow-lg text-black dark:bg-gray-900 dark:text-white z-50`}
      initial={{ x: "-100%" }}
                  ...
                  <div className="flex flex-col border-l-2 border-gray-400 dark:border-gray-600">
                    {category.links.map((link, idx) => (
                      <motion.div
                        ...
                        className="flex items-center space-x-2 pl-4 py-1 sm:py-1.5 hover:bg-gray-300 dark:hover:bg-gray-700 rounded-lg transition-colors duration-200"
                      >
                        <FaArrowRight className="text-xs text-gray-600 dark:text-gray-400" />
                        <Link href={link.href} onClick={onClose}>
                          <span className="hover:text-PrimaryButton cursor-pointer text-xs sm:text-base">
                            {link.name}
                          </span>
                        </Link>
                      </motion.div>
